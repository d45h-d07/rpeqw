...
The included `INOUT` program is not mine, but it's quite useful for translating between HP and PC text characters, and sharing content between emulators and physical devices. I found the code in this [HP Museum post](https://www.hpmuseum.org/forum/thread-13941-post-123629.html#pid123629).
...
## Motivation
Ever since the HP 48 series of calculators introduced RPL, its variable length interactive stack has had tremendous potential for enabling users to build and manipulate complex mathematical and programming objects.
...
4. Next, open the LIBRARY menu, select the appropriate port, and look for the soft menu entry `1726`
5. First press RSHIFT ALPHA(ENTRY), then hit the `1726` soft menu button to place its complete path on the command line, and then type ATTACH and press ENTER
...
### Install from source
The source code uses the Development lib functions →ALG and →LST, so this library must be attached first.
1. Switch to the HOME directory (otherwise the 'Development lib' app menu item will not appear)
2. If you have an older version of RPEQW attached as a library or folder, purge it now (after backing it up if you prefer)
3. Run the command `256 ATTACH`
4. Transfer the `rpeqw.rpl` source file to the calculator
...
6. Run the `CRLIB` command, and a library object with ID 1726 should appear on level 1 of the stack
